...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("M version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  md [options]                     " + "\n" +
                  "  md [options] <command> [params]  " + _("Send command to -server or md") + "\n" +
                  "  md [options] help                " + _("List commands") + "\n" +
                  "  md [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "m:"))
                fCommandLine = true;
