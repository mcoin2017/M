    ...
    // restored from backup or the user making copies of wallet.dat.
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                {
                    LogPrintf("WalletUpdateSpent found spent coin %s M %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
...
int64_t CWallet::GetTokenDebit(const CTxIn& txin) const
{
    if (!txin.IsAnonInput())
        return 0;

    // - amount of owned token decreased
...
int64_t CWallet::GetTokenCredit(const CTxOut& txout) const
{
    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                if (fUpdated)
                {
                    LogPrintf("ReacceptWalletTransactions found spent coin %s M %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                };
            } else
...
int64_t CWallet::GetTokenBalance() const
{
    int64_t nTotal = 0;

    {
        ...
        for (WalletTxMap::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            if (pcoin->IsTrusted() && pcoin->nVersion == ANON_TXN_VERSION)
                nTotal += pcoin->GetAvailableTokenCredit();
        };
    }
...
static int IsTokenCompromised(CTxDB *txdb, CPubKey &pubKey, CAnonOutput &ao, ec_point &vchSpentImage)
{
...
bool CWallet::ProcessAnonTransaction(CWalletDB *pwdb, CTxDB *ptxdb, const CTransaction& tx, const uint256& blockHash, bool& fIsMine, mapValue_t& mapNarr, std::vector<WalletTxMap::iterator>& vUpdatedTxns)
{
    ...
    for (uint32_t i = 0; i < tx.vin.size(); ++i)
    {
        ...
        for (uint32_t ri = 0; ri < (uint32_t)nRingSize; ++ri)
        {
            ...
            if (IsTokenCompromised(ptxdb, pkRingCoin, ao, vchImage) and Params().IsProtocolV3(nBestHeight))
...
bool CWallet::AddTokenInputs(int rsType, int64_t nTotalOut, int nRingSize, std::vector<std::pair<CScript, int64_t> >&vecSend, std::vector<std::pair<CScript, int64_t> >&vecChange, CWalletTx& wtxNew, int64_t& nFeeRequired, bool fTestOnly, std::string& sError)
{
    if (fDebugRingSig)
        LogPrintf("AddTokenInputs() %d, %d, rsType:%d\n", nTotalOut, nRingSize, rsType);
    ...
    for (std::vector<COwnedAnonOutput*>::iterator it = vPickedCoins.begin(); it != vPickedCoins.end(); ++it)
    {
        ...
        {
            LogPrintf("Error: SendTokenToToken() txin.scriptSig.resize threw: %s.\n", e.what());
        ...
        {
            LogPrintf("Error: SendTokenToToken() picked coin not in wallet, %s version %d.\n", (*it)->outpoint.hash.ToString().c_str(), (*mi).second.nVersion);
        ...
        if (!txout.IsAnonOutput())
        {
            sError = "picked coin not an token output.\n";
            return false;
        };
...
bool CWallet::SendMToToken(CStealthAddress& sxAddress, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee)
{
    if (fDebugRingSig)
        LogPrintf("SendMToToken()\n");
    ...
    if (vNodes.empty())
    {
        sError = _("Error: M is not connected!");
        return false;
    };
...
bool CWallet::SendTokenToToken(CStealthAddress& sxAddress, int64_t nValue, int nRingSize, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee)
{
    if (fDebugRingSig)
        LogPrintf("SendTokenToToken()\n");
    ...
    if (nValue + nTransactionFee > GetTokenBalance())
    {
        sError = "Insufficient Tokens";
        return false;
    };
    ...
    int64_t nFeeRequired;
    std::string sError2;
    if (!AddTokenInputs(nRingSize == 1 ? RING_SIG_1 : RING_SIG_2, nValue, nRingSize, vecSend, vecChange, wtxNew, nFeeRequired, false, sError2))
    {
        LogPrintf("SendTokenToToken() AddTokenInputs failed %s.\n", sError2.c_str());
        sError = "AddTokenInputs() failed : " + sError2;
        return false;
    };
...
bool CWallet::SendTokenToM(CStealthAddress& sxAddress, int64_t nValue, int nRingSize, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee)
{
    if (fDebug)
        LogPrintf("SendTokenToM()\n");
...
bool CWallet::ExpandLockedAnonOutput(CWalletDB *pwdb, CKeyID &ckeyId, CLockedAnonOutput &lao, std::set<uint256> &setUpdated)
{
    ...
    if (fDebugRingSig)
        LogPrintf("Adding token output to wallet: %s.\n", HexStr(pkImage).c_str());
...
bool CWallet::ProcessLockedAnonOutputs()
{
    ...
    // -- process owned token outputs received when wallet was locked.
...
bool CWallet::EstimateAnonFee(int64_t nValue, int nRingSize, std::string& sNarr, CWalletTx& wtxNew, int64_t& nFeeRet, std::string& sError)
{
    ...
    int64_t nFeeRequired;
    if (!AddTokenInputs(RING_SIG_2, nValue, nRingSize, vecSend, vecChange, wtxNew, nFeeRequired, true, sError))
    {
        LogPrintf("EstimateAnonFee() AddTokenInputs failed %s.\n", sError.c_str());
        sError = "AddTokenInputs() failed.";
        return false;
    };
...
int CWallet::CountAllAnonOutputs(std::list<CAnonOutputCount>& lOutputCounts, bool fMatureOnly)
{
    ...
    while (iterator->Valid())
    {
        ...
        if (!fProcessed)
            LogPrintf("WARNING: CountAllAnonOutputs found keyimage without matching token output value.\n");
...
bool CWallet::EraseAllAnonData()
{
    ...
    LogPrintf("Erasing token outputs.\n");
    ...
    LogPrintf("Erasing locked token outputs.\n");
    walletdb.EraseRange(std::string("lao"), nLao);
    LogPrintf("Erasing owned token outputs.\n");
    walletdb.EraseRange(std::string("oao"), nOao);
    LogPrintf("Erasing token output links.\n");
...
bool CWallet::InitBloomFilter()
{
    ...
    BOOST_FOREACH(const CKeyID &keyId, setKeys)
    {
        ...
        if (mi != mapAddressBook.end() && mi->second.compare(0, sAnonPrefix.length(), sAnonPrefix) == 0)
        {
            ...
            {
                CBitcoinAddress coinAddress(keyId);
                LogPrintf("InitBloomFilter() - ignoring key for token output %s.\n", coinAddress.ToString().c_str());
            };
            continue;
        };
...
bool CWallet::GetStakeWeightV1(int nHeight, const CKeyStore& keystore, uint64_t& nMinWeight, uint64_t& nMaxWeight, uint64_t& nWeight)
{
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
        ...
        int64_t nTimeWeight = GetWeight(nHeight, (int64_t)pcoin.first->nTime, (int64_t)GetTime());
...
void CWallet::FixSpentCoins(int& nMismatchFound, int64_t& nBalanceInQuestion, bool fCheckOnly)
{
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        for (unsigned int n=0; n < pcoin->vout.size(); n++)
        {
            ...
            {
                LogPrintf("FixSpentCoins found lost coin %s M %s[%d], %s\n",
            ...
            if (IsMine(pcoin->vout[n]) && !pcoin->IsSpent(n) && (txindex.vSpent.size() > n && !txindex.vSpent[n].IsNull()))
            {
                LogPrintf("FixSpentCoins found spent coin %s M %s[%d], %s\n",
                    FormatMoney(pcoin->vout[n].nValue).c_str(), pcoin->GetHash().ToString().c_str(), n, fCheckOnly? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
                if (!fCheckOnly)
