...
var qrcode = new QRCode("qrcode", {colorDark:'#3455AF', colorLight: '#ffffff', correctLevel: QRCode.CorrectLevel.H, width: 220, height: 220,});
    ...
    check: function(field)
    {
        ...
        for (var i = 0, len = value.length; i < len; ++i)
            if (base58.base58Chars.indexOf(value[i]) == -1) {
                el.css("background", "#3455AF").css("color", "white");
                return false;
            }
    ...
    $("#recipients").append((
        ...
        +  '<div id="recipient[count]" class="recipient"> \
            <div class="flex-right"> \
                ...
                <input id="pay_to[count]" class="pay_to input_box" data-title="The address to send the payment to  (e.g. XDN6TyyoWuXPJYaVtpMrRYRE5sxDtkBkhH)" placeholder="Enter a M address (e.g. XDN6TyyoWuXPJYaVtpMrRYRE5sxDtkBkhH)" maxlength="128" oninput="base58.check(this);" onchange="$(\'#label[count]\').val(bridge.getAddressLabel(this.value));"/> \
            ...
            <div class="flex-left"> \
                ...
                <select id="unit[count]" class="unit button is-inverse has-fixed-icon"  style="margin-left:10px; height:43px; width:100px;" onchange="unit.format(\'#amount[count]\', $(this).val());"> \
                    <option value="0" title="M"                    ' + (unit.type == 0 ? "selected" : "") + '>M</option> \
                    <option value="1" title="Milli-M (1 / 1000)"   ' + (unit.type == 1 ? "selected" : "") + '>mM</option> \
                    <option value="2" title="Micro-M (1 / 1000000)"' + (unit.type == 2 ? "selected" : "") + '>&micro;M</option> \
                    <option value="3" title="Shi-Ms (1 / 100000000)" ' + (unit.type == 3 ? "selected" : "") + '>sM</option> \
                </select> \
            </div> \
