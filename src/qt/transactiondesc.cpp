...
QString TransactionDesc::toHTML(CWallet *wallet, CWalletTx &wtx)
{
            ...
            BOOST_FOREACH(const CTxOut& txout, wtx.vout)
            {
                    ...
                    && txout.IsAnonOutput())
                {
                    ...
                    if (wallet->HaveKey(ckidD) && (wallet->mapAddressBook[ckidD].empty() || !wallet->mapAddressBook[ckidD].compare(0, sAnonPrefix.length(), sAnonPrefix) == 0))
                    {
                        ...
                        strHTML += "<br><br>";
                        strHTML += "<b>" + tr("To") + ":</b> <a href='"+explorer+"/Address/index.php?Currency=M&Address=";
    ...
    if (wtx.mapValue.count("to") && !wtx.mapValue["to"].empty())
    {
        ...
        strHTML += "<b>" + tr("To") + ":</b> <a href='"+explorer + "/Address/index.php?Currency=M&Address=" + GUIUtil::HtmlEscape(strAddress) + "' target='_blank'>";
    ...
    if (wtx.IsCoinBase() && nCredit == 0)
    {
        ...
        if (wtx.IsInMainChain())
            strHTML += BitcoinUnits::formatWithUnit(BitcoinUnits::M, nUnmatured)+ " (" + tr("matures in %n more block(s)", "", wtx.GetBlocksToMaturity()) + ")";
        else
    ...
    if (nNet > 0)
    {
        ...
        // Credit
        //
        strHTML += "<b>" + tr("Credit") + ":</b> " + BitcoinUnits::formatWithUnit(BitcoinUnits::M, nNet);
        strHTML += "<br><br>";
    } else
    {
        ...
        if (fAllFromMe)
        {
            ...
            BOOST_FOREACH(const CTxOut& txout, wtx.vout)
            {
                ...
                if (!wtx.mapValue.count("to") || wtx.mapValue["to"].empty())
                {
                    ...
                    {
                        strHTML += "<b>" + tr("To") + ":</b> <a href='"+explorer+"/Address/index.php?Currency=M&Address="+GUIUtil::HtmlEscape(CBitcoinAddress(address).ToString())+"' target='_blank'>";
                ...
                strHTML += "<b>" + tr("Debit") + ":</b> " + BitcoinUnits::formatWithUnit(BitcoinUnits::M, -txout.nValue);
                strHTML += "<br><br>";
            }
            ...
            if (fAllToMe)
            {
                ...
                strHTML += "<b>" + tr("Debit") + ":</b> " + BitcoinUnits::formatWithUnit(BitcoinUnits::M, -nValue);
                strHTML += "<br><br>";
                strHTML += "<b>" + tr("Credit") + ":</b> " + BitcoinUnits::formatWithUnit(BitcoinUnits::M, nValue);
                strHTML += "<br><br>";
            }

            int64_t nTxFee = nDebit - wtx.GetValueOut();
            if (nTxFee > 0)
                strHTML += "<b>" + tr("Transaction fee") + ":</b> " + BitcoinUnits::formatWithUnit(BitcoinUnits::M, -nTxFee);
                strHTML += "<br><br>";
        } else
        {
            //
            ...
            BOOST_FOREACH(const CTxIn& txin, wtx.vin)
            {
                ...
                if (wallet->IsMine(txin))
                    strHTML += "<b>" + tr("Debit") + ":</b> " + BitcoinUnits::formatWithUnit(BitcoinUnits::M, -wallet->GetDebit(txin)) + "<br><br>";
            };

            BOOST_FOREACH(const CTxOut& txout, wtx.vout)
                if (wallet->IsMine(txout))
                    strHTML += "<b>" + tr("Credit") + ":</b> " + BitcoinUnits::formatWithUnit(BitcoinUnits::M, wallet->GetCredit(txout)) + "<br><br>";
        }
    }

    strHTML += "<b>" + tr("Net amount") + ":</b> " + BitcoinUnits::formatWithUnit(BitcoinUnits::M, nNet, true) + "<br><br>";
    ...
    strHTML += "<b>" + tr("Transaction ID") + ":</b> <a href='"+explorer+"/Transaction/index.php?Currency=M&Transaction=" + txid + "' target='_blank'>" + txid + "</a><br>";
        ...
        BOOST_FOREACH(const CTxIn& txin, wtx.vin)
        {
            ...
            if (txdb.ReadDiskTx(prevout.hash, prev))
            {
                if (prevout.n < prev.vout.size())
                {
                    ...
                    {
                        strHTML +="<a href='"+explorer+"/Address/index.php?Currency=M&Address="+QString::fromStdString(CBitcoinAddress(address).ToString())+"' target='_blank'>";
                        if (wallet->mapAddressBook.count(address) && !wallet->mapAddressBook[address].empty())
                            strHTML += GUIUtil::HtmlEscape(wallet->mapAddressBook[address]) + " ";
                        strHTML += QString::fromStdString(CBitcoinAddress(address).ToString()) + "</a>";
                    }
                    strHTML = strHTML + " " + tr("Amount") + "=" + BitcoinUnits::formatWithUnit(BitcoinUnits::M, vout.nValue);
