// Copyright (c) 2014 The M developers
    ...
    nWeight(0)
{
    ...
    resize(1280, 720);
    setWindowTitle(tr("M") + " - " + tr("Client"));
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/m"));
    setWindowIcon(QIcon(":icons/m"));
#else
...
#ifdef Q_OS_WIN
    QFile html("C:/m/index.html");
#else
    QFile html("/opt/m/index.html");
#endif
...
void GUI::createActions()
{
    ...
    aboutAction = new QAction(QIcon(":/icons/bitcoin"), tr("&About M"), this);
    aboutAction->setToolTip(tr("Show information about M"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/trolltech/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for M"));
...
void GUI::setClientModel(ClientModel *clientModel)
{
    ...
    if (clientModel)
    {
        ...
        if (mode != NT_FULL)
        {
            ...
            setWindowTitle(tr("M") + " - " + tr("Wallet") + ", " + sMode);
        };
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/m_testnet"));
            setWindowIcon(QIcon(":icons/m_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/m_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("M client") + QString(" ") + tr("[testnet]"));
                trayIcon->setIcon(QIcon(":/icons/m_testnet"));
...
void GUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("M client"));
    trayIcon->setIcon(QIcon(":/icons/m"));
...
void GUI::setNumConnections(int count)
{
    ...
    connectionsIcon.setAttribute("data-title", tr("%n active connection(s) to M network", "", count));
}
    ...
    QString strMessage =
        tr("This transaction is over the size limit.  You can still send it for a fee of %1, "
          ...
          "Do you want to pay the fee?").arg(
                BitcoinUnits::formatWithUnit(BitcoinUnits::M, nFeeRequired));
...
void GUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid M address or malformed URI parameters."));
    }
...
#else
    walletModel->setWalletLockedOTP(true);
#endif
}
