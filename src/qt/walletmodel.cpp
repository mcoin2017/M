...
WalletModel::WalletModel(CWallet *wallet, OptionsModel *optionsModel, QObject *parent) :
    ...
    cachedBalance(0), cachedTokenBal(0), cachedStake(0), cachedUnconfirmedBalance(0), cachedImmatureBalance(0),
...
qint64 WalletModel::getTokenBalance() const
{
...
void WalletModel::checkBalanceChanged()
{
    ...
    qint64 newTokenBal = getTokenBalance();
    qint64 newStake = getStake();
    qint64 newUnconfirmedBalance = getUnconfirmedBalance();
    qint64 newImmatureBalance = getImmatureBalance();

    if (cachedBalance != newBalance
        || cachedTokenBal != newTokenBal
        ...
        || cachedImmatureBalance != newImmatureBalance)
    {
        ...
        cachedUnconfirmedBalance = newUnconfirmedBalance;
        cachedImmatureBalance = newImmatureBalance;
        emit balanceChanged(newBalance, newTokenBal, newStake, newUnconfirmedBalance, newImmatureBalance);
    }
}
...
WalletModel::SendCoinsReturn WalletModel::sendCoinsAnon(const QList<SendCoinsRecipient> &recipients, const CCoinControl *coinControl)
{
    ...
    if (vNodes.empty())
        return SendCoinsReturn(SCR_ErrorWithMsg, 0, QString::fromStdString("M is not connected!"));
    ...
    {
        nBalance = wallet->GetTokenBalance();
        if ((nTotalOut + MIN_TX_FEE_ANON) > nBalance)
            return SendCoinsReturn(SCR_AmountWithFeeExceedsTokenBalance, MIN_TX_FEE_ANON);
    };

    {
        ...
        foreach(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            if (rcp.txnTypeInd == TXT_M_TO_M
                || rcp.txnTypeInd == TXT_ANON_TO_M)
            {
                // -- out m
                std::string sError;
        ...
        } else
        {
            ...
            std::string sError;
            if (!wallet->AddTokenInputs(RING_SIG_2, nTotalOut, nRingSize, vecSend, vecChange, wtxNew, nFeeRequired, false, sError))
            {
                if ((nTotalOut + nFeeRequired) > nBalance) // FIXME: could cause collisions in the future
                    return SendCoinsReturn(AmountWithFeeExceedsBalance, nFeeRequired);
...
bool WalletModel::setWalletEncryptedOTP(bool encrypted, const SecureString &passphrase, const QByteArray otp_secret, const SecureString &veri_otpcode, const bool otp_enable)
{
    if(encrypted)
    {
        // Encrypt
        return wallet->EncryptWalletOTP(passphrase, otp_secret, veri_otpcode, otp_enable);
    }
    else
    {
        // Decrypt -- TODO; not supported yet
...
bool WalletModel::setWalletLockedOTP(bool locked, const SecureString &passPhrase, const SecureString &otpcode, bool stakingOnly)
{
    if(locked)
    {
        // Lock
    ...
    {
        // Unlock
        return wallet->UnlockOTP(passPhrase, otpcode, stakingOnly);
    }
}

bool WalletModel::changePassphraseOTP(const SecureString &oldPass, const SecureString &newPass, const SecureString &otpcode, const QByteArray otp_secret, const SecureString &veri_otpcode, const bool otp_enable)
{
    bool retval;
    {
        LOCK(wallet->cs_wallet);
        wallet->Lock(); // Make sure wallet is locked before attempting pass change
        retval = wallet->ChangeWalletPassphraseOTP(oldPass, newPass, otpcode, otp_secret, veri_otpcode, otp_enable);
    }
    return retval;
}
...
#else
       wallet->setWalletLockedOTP(true);
#endif
    }
}
