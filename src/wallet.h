...
public:
    ...
    bool GetStakeWeightV1(int nHeight, const CKeyStore& keystore, uint64_t& nMinWeight, uint64_t& nMaxWeight, uint64_t& nWeight);
    ...
    bool AddTokenInputs(int rsType, int64_t nTotalOut, int nRingSize, std::vector<std::pair<CScript, int64_t> >&vecSend, std::vector<std::pair<CScript, int64_t> >&vecChange, CWalletTx& wtxNew, int64_t& nFeeRequired, bool fTestOnly, std::string& sError);
    bool SendMToToken(CStealthAddress& sxAddress, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee=false);
    bool SendTokenToToken(CStealthAddress& sxAddress, int64_t nValue, int nRingSize, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee=false);
    bool SendTokenToM(CStealthAddress& sxAddress, int64_t nValue, int nRingSize, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee=false);
    ...
    int64_t GetTokenDebit(const CTxIn& txin) const;
    int64_t GetTokenCredit(const CTxOut& txout) const;
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nM, int64_t& nToken) const
    {
        nM = 0;
        nToken = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
...
public:
    ...
    mutable bool fAvailableTokenCreditCached;
    mutable bool fCreditSplitCached;
    mutable int64_t nDebitCached;
    mutable int64_t nCreditCached;
    mutable int64_t nAvailableCreditCached;
    mutable int64_t nChangeCached;
    mutable int64_t nAvailableTokenCreditCached;
    
    mutable int64_t nCredMCached;
    mutable int64_t nCredTokenCached;
    ...
    bool GetCredit(int64_t& nCredM, int64_t& nCredToken, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredMCached = 0;
            nCredTokenCached = 0;
            pwallet->GetCredit(*this, nCredMCached, nCredTokenCached);
            fCreditSplitCached = true;
        };
        
        nCredM = nCredMCached;
        nCredToken = nCredTokenCached;
        return true;
    }
    ...
    int64_t GetAvailableTokenCredit(bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
            return 0;
        
        if (fUseCache && fAvailableTokenCreditCached)
            return nAvailableTokenCreditCached;
        ...
        nAvailableTokenCreditCached = nCredit;
        fAvailableTokenCreditCached = true;
        return nCredit;
    };
